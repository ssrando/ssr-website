...
import { ServerActionError, ServerActionResult } from "../../../controller/ServerAction";

const EditData = () => {
    const { name } = useParams();

    const { data, error, isLoading } = useGetApi<DynamicData[]>(`/api/dynamicdata/${name}`);
    ...
    const loadEditor = useCallback((dataIndex: number) => {
        ...
        <>
            <Box sx={{ display: "flex"}}>
                <Box sx={{ position: "absolute" }}>
                    <Button onClick={() => navigate('/admin/dynamicdata')}>Back</Button>
                </Box>
                <Typography sx={{ flexGrow: 1 }} variant="h4">{name}</Typography>
            </Box>
            
            {
                data.map((item, index) => (
                    <Button key={item.id} variant="contained" onClick={() => loadEditor(index)}>{name} {index}</Button>
                ))
            }
            { 
                showEditor && <JSONEditorComponent content={content} onChange={setContent} />
            }
            <Box sx={{ display: "flex", flexDirection: "column" }}>
                <Box sx={{ flexGrow: 1 }} />
            </Box>
            <Button variant="contained" onClick={discard}>Discard</Button>
